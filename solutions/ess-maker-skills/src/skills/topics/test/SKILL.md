...
- **Only the eval cases are read-only.** Fixing the topic is the goal, so editing it — or its flow / template config — is expected, intentional mutation. The eval cases are what you never touch (the guardrail). The supporting diagnostics are non-destructive too: flow-run inspection and the Topic checker only read, and a planted DBG node is the one *temporary* topic mutation — byte-reversible and always stripped.
