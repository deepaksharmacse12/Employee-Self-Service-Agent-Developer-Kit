...
class DiscoveryRunner:
    ...
    def run(self, environment_ids: list[str] | None = None) -> RunSummary:
    ...
    def _map_and_upsert(
        ...
        for resource in resources:
            ...
            try:
                item = map_resource(kind, resource, caps=self._config.caps)
    ...
    def _upsert_all(self, items: list[InventoryItem], report: ScopeReport) -> None:
        ...
        applied -- not a failure (§5.2). Natural keys of successfully-applied items are
        recorded in ``report.observed_keys`` -- the snapshot the server diffs against to
        retire drift (set-based reconcile, replacing the RunId watermark).
        """
        if not items:
            return
